 ...
 *
 * API version: v2beta1.19 (recommended default)
...
func (s *Service) ListMemberPermissions(member string, query *ListMemberPermissionsQueryParams, resp ...*http.Response) ([]string, error) {
	...
	u, err := s.Client.BuildURLFromPathParams(values, serviceCluster, `/identity/v2beta1/members/{{.Member}}/permissions`, pp)
	if err != nil {
		return nil, err
	}
	response, err := s.Client.Get(services.RequestParams{URL: u})
...
/*
	...
	Parameters:
		...
		resp: an optional pointer to a http.Response to be populated by this method. NOTE: only the first resp pointer will be used if multiple are provided
*/
func (s *Service) SetPrincipalPublicKeys(principal string, resp ...*http.Response) error {
	pp := struct {
		Principal string
	}{
		Principal: principal,
	}
	u, err := s.Client.BuildURLFromPathParams(nil, serviceCluster, `/system/identity/v2beta1/principals/{{.Principal}}/keys`, pp)
	if err != nil {
		return err
	}
	response, err := s.Client.Put(services.RequestParams{URL: u})
	if response != nil {
		defer response.Body.Close()
