...
####################################################################################################
# Push changes to splunkbeta Github master branch
####################################################################################################
OWNER=splunkbeta
PROJECT=splunk-cloud-sdk-go
echo "Pushing to github.com/${OWNER}/${PROJECT} ..."
# Remove git settings
rm -rf .git
# Setup git for service user
git init
git config user.name "splunk-srv-dev-platform"
git config user.email "srv-dev-platform@splunk.com"
set +x
git remote add origin "https://splunk-srv-dev-platform:${GITHUB_TOKEN}@github.com/${OWNER}/${PROJECT}.git"
set -x
# .gitignore all cicd files
printf "\n" >> .gitignore
cat .splunkbeta.gitignore >> .gitignore
# Push changes to splunkbeta
git add .
git commit -m "Release v${NEW_VERSION}"
git push --set-upstream origin master --force
SPLUNKBETA_SHA=$(git rev-parse HEAD)
####################################################################################################
# Create a tag and release for splunkbeta Github
####################################################################################################
echo "Creating a tag and release on ${OWNER}/${PROJECT} ..."
set +x
RELEASE_RESPONSE=$(curl -s -H "Authorization: token $GITHUB_TOKEN" "https://api.github.com/repos/${OWNER}/${PROJECT}/releases" -d "{\"tag_name\": \"v${NEW_VERSION}\", \"target_commitish\": \"${SPLUNKBETA_SHA}\", \"name\": \"Release v${NEW_VERSION}\", \"body\": \"${RELEASE_NOTES}\"}")
RELEASE_EXIT_CODE=$?
set -x
echo "Release response: ${RELEASE_RESPONSE}"