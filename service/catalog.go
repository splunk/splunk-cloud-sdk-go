...
func (c *CatalogService) GetDataset(name string) (*model.Dataset, error) {
	url, err := c.client.BuildURL(nil, catalogServicePrefix, catalogServiceVersion, "datasets", name)
	if err != nil {
		return nil, err
	}
	response, err := c.client.Get(url)
...
func (c *CatalogService) DeleteDataset(datasetName string) error {
	url, err := c.client.BuildURL(nil, catalogServicePrefix, catalogServiceVersion, "datasets", datasetName)
	if err != nil {
		return err
	}
...
func (c *CatalogService) DeleteRule(rulePath string) error {
	getDeleteURL, err := c.client.BuildURL(nil, catalogServicePrefix, catalogServiceVersion, "rules", rulePath)
	if err != nil {
		return err
	}
